...
//    char *argv[] = {"", "", "", ""};
//    int argc = 6;
////    char *argv[] = {"udp://@192.168.1.3:8905?fifo_size=1000000&overrun_nonfatal=1&buffer_size=102400&pkt_size=102400"};
...
//        fprintf(stderr, "Use -h to get full help or, even better, run 'man %s'\n", program_name);
    ...
    NSLog(@"这个执行了么？>>");
    if (display_disable) {NSLog(@"这个执行了么？");
        video_disable = 1;
    } else {
        video_disable = 0;
    }
...
//    
//    av_init_packet(&flush_pkt);
//    flush_pkt.data = (UInt8 *)(intptr_t)"FLUSH";
//    
//    is = stream_open(input_filename, file_iformat);
//    if (!is) {
//        fprintf(stderr, "Failed to initialize VideoState!\n");
...
//        calculate_display_rect(&rect, is->xleft, is->ytop, is->width, is->height, vp);
//        
////        rect.y = -300;rect.w = 300;rect.h = 300;
//        
//        NSLog(@"rect.x:%d %d %d %d", rect.x, rect.y, rect.w, rect.h);
//        SDL_DisplayYUVOverlay(vp->bmp, &rect);
//        
...
//            packet_queue_start(&is->subtitleq);
//            
//            is->subtitle_tid = SDL_CreateThread(subtitle_thread, is->filename, is);
//            break;
//        default:
//            break;
//    }
...
//        ret = stream_component_open(is, st_index[AVMEDIA_TYPE_VIDEO]);
//    }
//    is->refresh_tid = SDL_CreateThread(refresh_thread, is->filename, is);
//    if (is->show_mode == SHOW_MODE_NONE)
//        is->show_mode = ret >= 0 ? SHOW_MODE_VIDEO : SHOW_MODE_RDFT;
//    
//    if (st_index[AVMEDIA_TYPE_SUBTITLE] >= 0) {
...
//    is->av_sync_type = av_sync_type;
//    is->read_tid     = SDL_CreateThread(read_thread, is->filename, is);
//    if (!is->read_tid) {
//        av_free(is);
//        return NULL;
//    }
